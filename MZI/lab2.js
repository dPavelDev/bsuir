...
let AES_Sbox_Inv, AES_ShiftRowTab_Inv, AES_xtime;
...
function AES_ExpandKey(key) {
  ...
  switch (kl) {
    ...
    default:
      alert("Ключ может быть только 16, 24 или 32 байта!");
  }
  ...
  for(let i = kl; i < ks; i += 4) {
    ...
    if (i % kl === 0) {
      ...
      temp = [
        AES_Sbox[temp[0]], AES_Sbox[temp[1]],
...
AES_Init();
const block = ['a', 'b', 'c'].map(char => char.charCodeAt(0));
...
console.log(block.map(char => String.fromCharCode(char)));
// AES_Done();