...
class IntegerLinearProgrammingSolver(object):
  ...
  def recursion(self, d_low, d_high, basic_indexes = None, nonbasic_indexes = None):
    try:
    ...
    if (self.is_integers(x)):
      ...
      right = copy(d_high)
      right[j0] = value
      self.recursion(d_low, right, res.basic_indexes, res.nonbasic_indexes)

      left = copy(d_low)
      left[j0] = value + 1
      self.recursion(left, d_high, res.basic_indexes, res.nonbasic_indexes)

  def the_branch_and_bound_method(self):
    self.recursion(self.d_low, self.d_high)
